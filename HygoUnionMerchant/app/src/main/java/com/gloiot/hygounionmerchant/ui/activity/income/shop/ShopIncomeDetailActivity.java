...
/**
 * 小铺--统计--收益明细
 * Created by Dlt on 2017/8/24 9:50
 */
public class ShopIncomeDetailActivity extends BaseActivity implements PullToRefreshLayout.OnRefreshListener {

    @Bind(R.id.tv_start_time)
    TextView mTvStartTime;
    @Bind(R.id.tv_end_time)
    TextView mTvEndTime;
    @Bind(R.id.tv_money_jiaoyi)
    TextView mTvMoneyJiaoyi;
...
//        month = intent.getStringExtra("month");
    ...
    private void setAdapter() {
        ...
        mCommonAdapter = new CommonAdapter<String[]>(ShopIncomeDetailActivity.this, R.layout.item_shop_order_base_style, list) {
            @Override
            public void convert(final ViewHolder holder, String[] strings) {

                holder.setText(R.id.tv_order_num, "交易单号：" + strings[0]);
        ...
        mPullablelistview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                Intent intent = new Intent(ShopIncomeDetailActivity.this, ShopOrderDetailActivity.class);
    ...
    private void processResponseData(JSONObject response, boolean isLoadMore) throws JSONException {
        L.e("小铺/酒店/旅游月份收益明细", response.toString());
        String monthTotalDaozhangIncome = response.getString("本月收益到账总金额");
        String monthTotalJiaoyiIncome = response.getString("本月收益交易总金额");
        String endTime = response.getString("最大日期");
        String startTime = response.getString("最小日期");

        mTvMoneyDaozhang.setText(monthTotalDaozhangIncome);
        mTvMoneyJiaoyi.setText(monthTotalJiaoyiIncome);
